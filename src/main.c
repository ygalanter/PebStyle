...
static void info_update_proc(Layer *layer, GContext *ctx) {
  ...
  if (flag_locationService != LOCATION_DISABLED) {
    ...
    graphics_draw_text(ctx, s_temp, font_18, GRect(bounds.origin.x + 5,bounds.size.h - 20, 28, 20), GTextOverflowModeFill, GTextAlignmentCenter, NULL);
    ...
    #endif
    if (meteoicon_current) graphics_draw_bitmap_in_rect(ctx, meteoicon_current, GRect(bounds.origin.x + 5,bounds.size.h - 41, ICON_WIDTH, ICON_HEIGHT));
